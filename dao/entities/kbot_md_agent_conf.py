...
class KbotMdAgentConf(Base):
    ...
    agent_id: Mapped[int] = mapped_column(Numeric(38, 0), comment="智能体ID")
    tool_id: Mapped[int] = mapped_column(Numeric(38, 0), comment="知识库ID/FUNC_ID等")
    tool_type: Mapped[int] = mapped_column(Numeric(2, 0), comment="枚举类型")
    tool_weight: Mapped[float | None] = mapped_column(Numeric(2, 2), comment="知识库检索权重")
    reranker_flag: Mapped[int | None] = mapped_column(Numeric(1, 0), comment="是否需要重排")
    search_type: Mapped[int] = mapped_column(Numeric(2, 0), comment="搜索类型枚举类型")
