...
class OpsDbInstanceEntity(BaseEntity):
    ...
    monitor_type: Mapped[str] = mapped_column(String(32), default="prometheus", comment="监控数据源类型: prometheus, zabbix, oem, none")
    ...
    oem_target_name: Mapped[str | None] = mapped_column(String(256), comment="Oracle Enterprise Manager 中的目标名称（targetName）")
    oem_target_type: Mapped[str | None] = mapped_column(String(64), default="oracle_database", comment="OEM 目标类型（如 oracle_database, host, listener）")
