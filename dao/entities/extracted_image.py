...
class ExtractedImageEntity(BaseEntity):
    ...
    image_path: Mapped[str] = mapped_column(String(1000), nullable=False, comment="提取图片的存储路径")
    embedding: Mapped[list[float] | None] = mapped_column(VectorField(), comment="ColQwen2 mean-pooled 视觉 embedding（全精度 vector）")
    description: Mapped[str | None] = mapped_column(Text, comment="VLM 对该图片的描述文本")
    image_type: Mapped[str] = mapped_column(String(32), default="figure", comment="图片类型: figure/table/chart/photo")
    bbox: Mapped[dict | None] = mapped_column(OracleJSON, comment="图片在页面中的边界框坐标")
    chunk_id: Mapped[str | None] = mapped_column(String(256), comment="关联的文本 chunk ID")
