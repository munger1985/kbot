...
from sqlalchemy import String, CLOB, Integer, ForeignKey, func, Numeric
...
class GraphVertexEntity(BaseEntity):
    ...
    attributes: Mapped[dict | None] = mapped_column(OracleJSON, name="ATTRIBUTES", comment="JSON格式动态扩展属性，存储非固定字段")
...
class GraphEdgeEntity(BaseEntity):
    ...
    attributes: Mapped[dict | None] = mapped_column(OracleJSON, name="ATTRIBUTES", comment="JSON格式动态扩展属性，存储边特有附加信息")
