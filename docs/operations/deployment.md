...
## 下载 BGE Reranker ONNX INT8 模型

先完成工作区依赖安装，再在实际运行 Reranker 进程的模型服务器上执行：

```bash
conda run -n kbot4 python \
  scripts/deployment/models/download_bge_reranker_int8.py \
  --model-dir /models/bge-reranker-v2-m3-onnx-int8
```

如使用的 Conda 环境不是 `kbot4`，应替换为实际环境名。`--model-dir` 应指向模型服务
账号可读的持久化绝对路径；不要放入 Git 工作区或临时目录。服务器需要能够访问
Hugging Face 公共模型仓库，模型文件约为 571 MB。

脚本固定下载 `onnx-community/bge-reranker-v2-m3-ONNX` 的完整 revision，只获取
tokenizer、模型配置和 `onnx/model_int8.onnx`，随后校验模型架构及 SHA-256。成功后
目录至少包含：
...
└── kbot_artifact_source.json
```

摘要不匹配时脚本会拒绝生成运行清单。确认网络和存储没有损坏后，可显式重新下载：
...
下载只准备本地制品，不会写入 `KBOT_AI_MODEL`。随后通过模型配置页面登记：

```text
Category: 4
Provider: local_bge_onnx_int8
Provider Model Name: BAAI/bge-reranker-v2-m3
Served Model Name: bge-reranker-cpu
API Endpoint: 留空
API Key: 留空
```
...
```json
{
  "model_path": "/models/bge-reranker-v2-m3-onnx-int8",
  "manifest_path": "reranker_manifest.json",
  "max_tokens": 512,
  "batch_size": 8,
  "cpu_threads": 8,
...
模型先以 `DRAFT` 登记，验证真实推理后再切换为 `ACTIVE`。Reranker 绑定在 Knowledge
Core Collection 的 `models.reranker` 角色上，不直接绑定 Agent；Agent 通过 Collection
Binding 使用各 Collection 自己的 Reranker。量化模型的分数分布可能不同，正式切换前
必须用业务标注集重新校准两个阈值。
