...
**目标：** 用数据证明 V2 链路质量与可靠性，并完成首批来源/App/Agent 的切流。

**交付物：** 标注集、离线评测报告、压测/故障演练、存量知识重新入库或迁移工具、V1/V2 流量开关、上线与回滚（路由/KC Version/部署级）方案。

**完成门槛：** 达成预先冻结的 Discovery/Evidence Recall@K、引用定位、跨附件覆盖、解析时延和失败率目标；首批范围稳定切至 V2，V1/V2 数据、API、Skill 与审计日志保持隔离。旧 `File/Chunk` 的最终下线不属于本阶段完成条件。
