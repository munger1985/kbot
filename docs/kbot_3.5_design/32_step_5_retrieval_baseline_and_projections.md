...
它不包含 Bundle 标题、Revision Facet、Member 声明名称、角色或 LLM 文档摘要，因为这些字段可随 Revision 改变。`INDEX` Job 对该文本生成 Oracle Text 索引与 embedding，记录 `embedding_model_key`、`retrieval_profile_version` 和输入 hash。重跑相同 Version/View/规则/hash 幂等；模型或规则变化创建新的索引产物并在验证后切换。
...
4. 每个 Collection 使用同一候选预算和最低相关性阈值，随后跨 Collection 再做 RRF/校准分数与结果多样化；未达阈值的 Collection 不强制占位。
5. 输出 Bundle/Document Member candidate，不直接输出 Evidence。
...
4. 不默认使用 Relation 或 LLM rerank；它们是后续受评测开关。基线仅保留命中理由、融合分数和扩展来源供调试/离线评测。

## 参数、版本与可观测性

Collection 可保存受控 `retrieval_policy_key`，指向版本化策略配置：全文/vector 候选数、RRF 常数、最低阈值、邻接窗口、每 Document 上限、总 token 预算和是否启用特定 ViewType。Agent Binding 不保存这些参数，保持 Collection 平权和策略集中治理。
