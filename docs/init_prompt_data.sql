...
BEGIN
    ...
    -- 提示词: SYSTEM/image2text
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/image2text' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/rewrite_question
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/rewrite_question' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/refresh_summary
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/refresh_summary' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/rag_final_render
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/rag_final_render' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '{prompt}
...
',
    ...
    -- 提示词: SYSTEM/user_profile
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/user_profile' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/sql_gen
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/sql_gen' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/sql_repair
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/sql_repair' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/task_planner
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/task_planner' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/intent_router
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/intent_router' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/reasoning
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/reasoning' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/generate_chart
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/generate_chart' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/db_router
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/db_router' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/graph_extractor
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/graph_extractor' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '你是一个顶级的企业级知识图谱抽取专家。
...
{{
  ...
  "edges": [
    ...
    -- 提示词: SYSTEM/graph_vertex_fusion
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/graph_vertex_fusion' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '你是一个知识图谱专家，正在维护一个全面、无冗余的实体百科全书。
...
3. 直接输出融合后的完整最终描述，不要包含任何前导语、后缀或解释性文字。',
    ...
    WHEN NOT MATCHED THEN
        INSERT (
            ...
            1001,
            NULL,
            'SYSTEM/graph_vertex_fusion',
            'SYSTEM/graph_vertex_fusion',
            0,
            '你是一个知识图谱专家，正在维护一个全面、无冗余的实体百科全书。
请将关于实体【{name}】（类型：{v_type}）的新增上下文信息，自然地融合进原有的百科描述中。
...
3. 直接输出融合后的完整最终描述，不要包含任何前导语、后缀或解释性文字。',
    ...
    -- 提示词: SYSTEM/ops_rewrite
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/ops_rewrite' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/ops_diagnosis
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/ops_diagnosis' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
...
',
    ...
    -- 提示词: SYSTEM/ops_planner
    MERGE INTO KBOT_MD_PROMPT p
    USING (SELECT 'SYSTEM/ops_planner' AS PROMPT_UNIQUE_NAME FROM DUAL) src
    ON (p.PROMPT_UNIQUE_NAME = src.PROMPT_UNIQUE_NAME)
    WHEN MATCHED THEN
        UPDATE SET
            TEMPLATE = '
