...
规范建库脚本只面向空 Schema。已有开发库升级前必须备份并停服，再执行
`scripts/db/migrate_access_scope_model.sql`。该脚本迁移现有成员与 Domain 范围，但不会
猜测哪个旧用户应成为初始 App 管理员。迁移完成后由 ADMIN 登录平台入口，逐 App 建立
App-Domain 关系并创建初始管理员，再开放业务登录。
