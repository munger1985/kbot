...
`PARTIAL`不等于结束对话。如果未取得的证据会影响用户当前问题，回答在说明已知
结论后，还必须给出可重放的只读SQL或PromQL、所需权限或配置项，并明确告诉
用户修复或粘贴结果后可在下一轮继续同一次诊断。
...
`DATABASE_OVERVIEW`不是单次数据库快照的同义词。Turn Planner必须在同一任务DAG中并行
...
监控绑定、查询窗口、指标目录、查询覆盖和凭据引用必须在Turn规划事务内冻结。每个绑定
