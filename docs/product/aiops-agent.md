...
窗口、Tool Allowlist、查询预算、平台Secret隔离、引用完整性、根因等级上限和执行策略。每项
...
Tool Allowlist、采集边界、平台Secret隔离、引用校验、根因等级上限和执行策略。授权日志
正文和数据库字典值保持原样，访问范围由数据源绑定与诊断数据库账号决定。
...
- Agent 按 Target 保存 `controlled_action_execution`，用户必须显式选择允许的已登记动作；
  Catalog 后续新增动作不会自动获得授权；
- 没有执行凭据、部署级开关关闭、动作未选择或当前没有兼容执行器时，不生成可审批执行入口；
- 执行资源完整时，也只能逐条审批后执行已选择的非破坏性登记动作。

执行策略由 Agent 表单按 Target 生成并随 Agent 版本化。页面只展示与 Target 数据库类型、
版本、能力和环境兼容的动作，但是否授权必须由用户逐项选择，并可限制 Schema 和每日执行
次数。自动告警诊断开关、最低级别和冷却时间只影响监控告警触发，不改变 Chat、人工 Run
或计划巡检的行为。
