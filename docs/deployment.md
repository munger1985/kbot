...
# 默认放到 <data_dir>/models/docling_models；其他路径写入 kbot.toml 的 paths.docling_models
```
...
## 6 准备 KBot 部署配置
**说明**：
- 部署时只需复制 `configuration/kbot.toml.example` 为 `kbot.toml`，填写数据库、
  数据目录、日志目录、向量维度和 Portal API Key 摘要；内部服务
  参数使用代码默认值。
- 复制 `.env.example` 为受保护的环境文件，至少设置 Oracle 密码和
  `KBOT_MASTER_KEY`。具体步骤见 `configuration/README.md`。
...
| 症状 | 排查步骤 |
|---|---|
| 服务启动失败 | 检查 `var/log/` 目录日志，确认 conda 环境 `kbot3` 正确激活 |
