...
CREATE SEARCH INDEX idx_ops_rep_json
    ON kbot.ops_execution_report(actions_executed)
    FOR JSON;
...
COMMENT ON COLUMN kbot.ops_execution_report.recommendations IS 'LLM 生成的后续优化建议';
COMMENT ON COLUMN kbot.ops_execution_report.actions_executed IS '执行的变更动作列表 [{sql, impact, risk_level}]';
