...
公共 Shell 先注入完整侧栏与顶栏，再按权限删除不可访问项目，不能仅置灰。`GET /access` 失败时仍保留 chrome 并展示错误，不能让整栏消失。当前入口使用 `aria-current="page"`；顶栏显示当前业务 Token 固定的 Domain，并提供“切换 Domain”入口（仅列出当前 App 授权范围）。
