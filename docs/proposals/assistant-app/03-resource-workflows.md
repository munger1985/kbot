...
→ 测试问题 → 审核 → 发布 → 在 Agent 中选择
```

列表显示名称、Domain、数据源、版本和 `DRAFT/REVIEW/PUBLISHED/ARCHIVED` 状态。只有 `PUBLISHED` 版本可以在 Agent 向导中选取；草稿修改创建新版本，不能原地改变已运行 Run 所依赖的口径。
...
3. **问数范围**：选择零至多个 `PUBLISHED` 问数模型，保存时自动同步当前 Agent 版本的内部投影；
...
| KC 停用、问数模型撤销发布 | Agent 自动不可用或降级，用户获得明确原因 |
