...
class OracleDB:
    ...
    async def query(
        ...
        async with self.pool.acquire() as connection:
            ...
            logger.debug(f"OracleDB query method: {sql}, {params}")
    ...
    async def execute(self, sql: str, data: Union[list, dict] = None):
        ...
        knowledge_base = os.environ.get("ORACLE_WORKSPACE")
        # 如果环境变量存在，则替换 params 中 workspace 的值
        if knowledge_base and data and 'workspace' in data:
            data['workspace'] = knowledge_base

        logger.debug(f"OracleDB execute method: {sql}, {data}")
        try:
            async with self.pool.acquire() as connection:
                connection.inputtypehandler = self.input_type_handler
                connection.outputtypehandler = self.output_type_handler
...
class OracleKVStorage(BaseKVStorage):
    ...
    async def filter_keys(self, keys: set[str]) -> set[str]:
        ...
        #print(f"filter_keys from {self.namespace}: {SQL},{params} ")
        res = await self.db.query(SQL, params, multirows=True)
        if res:
...
class OracleDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        sql = "SELECT * FROM LIGHTRAG_DOC_STATUS WHERE workspace=:workspace AND id in (:ids)"
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        sql = """MERGE INTO LIGHTRAG_DOC_STATUS
                USING DUAL ON (id = :id and workspace = :workspace)
                WHEN MATCHED THEN UPDATE SET content_length = :content_length, 
                chunks_count = :chunks_count, status = :status, updated_at = SYSDATE
                WHEN NOT MATCHED THEN INSERT(workspace,id,content,content_summary,content_length,chunks_count,status,file_path)
                values(:workspace,:id,:content,:content_summary,:content_length,:chunks_count,:status,:file_path)
              """
...
class OracleGraphStorage(BaseGraphStorage):
    ...
    async def upsert_node(self, node_id: str, node_data: dict[str, str]) -> None:
        ...
        source_chunk_id = node_data["source_id"]
        file_path = node_data["file_path"]
        logger.info(f"entity_name:{entity_name}, entity_type:{entity_type}, file_path:{file_path}")
        ...
        #print("GraphDBStroage merge node sql: %s , %s " % (merge_sql, data))
    ...
    async def upsert_edge(
        ...
        logger.info(
            f"source_name:{source_name}, target_name:{target_name}, file_path: {file_path}"
        )
       ...
       # print("GraphDBStroage merge edge sql: %s ,%s " % (merge_sql, data))
        await self.db.execute(merge_sql, data)
    ...
    async def get_knowledge_graph(
        ...
        source_str = "', '".join(node_names)
        target_str = "', '".join(node_names)

        sql = f"""
            SELECT source_name, target_name, weight, keywords, description, source_chunk_id 
            FROM LIGHTRAG_GRAPH_EDGES
            WHERE workspace = :workspace
            AND (source_name IN ('{source_str}') OR target_name IN ('{target_str}'))
        """
        # Get related edges
        edge_results = await self.db.query(
            sql,
            #SQL_TEMPLATES["get_related_edges"],
            {"workspace": self.db.workspace},
            multirows=True,
        )

        if not edge_results:
            logger.warning(f"No edge found matching workspace: {self.db.workspace}")
...
TABLES = {
    ...
    "LIGHTRAG_GRAPH_NODES": {
        "ddl": """CREATE TABLE LIGHTRAG_GRAPH_NODES (
                    ...
                    workspace varchar2(1024),
                    name varchar2(2048),
                    entity_type varchar2(1024),
                    description CLOB,
                    source_chunk_id varchar2(2048),
                    content CLOB,
                    content_vector VECTOR,
                    createtime TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    updatetime TIMESTAMP DEFAULT NULL,
    ...
    "LIGHTRAG_GRAPH_EDGES": {
        "ddl": """CREATE TABLE LIGHTRAG_GRAPH_EDGES (
                    id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    workspace varchar2(1024),
                    source_name varchar2(2048),
                    target_name varchar2(2048),
                    weight NUMBER,
                    keywords CLOB,
                    description CLOB,
                    source_chunk_id varchar2(2048),
...
SQL_TEMPLATES = {
    # SQL for DocStatusStorage
    "merge_docs_status": """MERGE INTO LIGHTRAG_DOC_STATUS
        USING DUAL
        ...
        WHEN NOT MATCHED THEN INSERT
            (workspace,id,content,content_summary,content_length,chunks_count,status,file_path)
            values (:workspace,:id,:content,:content_summary,:content_length,:chunks_count,:status,:file_path) """,
    ...
    "get_by_status_ids_text_chunks": "select id,status from LIGHTRAG_DOC_CHUNKS where workspace=:workspace and status=:status and ID in ({ids})",
    ...
    "merge_chunk": """MERGE INTO LIGHTRAG_DOC_CHUNKS
        ...
        ON (id = :id and workspace = :workspace)
        WHEN NOT MATCHED THEN INSERT
            (id,workspace,tokens,chunk_order_index,full_doc_id,content,content_vector,file_path)
            values (:id,:workspace,:tokens,:chunk_order_index,:full_doc_id,:content,:content_vector,:file_path) """,
    ...
    "chunks": """SELECT id FROM
        (SELECT id,VECTOR_DISTANCE(content_vector,vector(:embedding_string),COSINE) as distance
    ...
    "merge_node": """MERGE INTO LIGHTRAG_GRAPH_NODES a
                ...
                WHEN NOT MATCHED THEN
                    INSERT(workspace,name,entity_type,description,source_chunk_id,content,content_vector,file_path)
                    values (:workspace,:name,:entity_type,:description,:source_chunk_id,:content,:content_vector,:file_path)
                WHEN MATCHED THEN
                    UPDATE SET
                    entity_type=:entity_type,description=:description,
                    source_chunk_id=:source_chunk_id,content=:content,
                    content_vector=:content_vector,updatetime=SYSDATE,
                    file_path=:file_path""",
    "merge_edge": """MERGE INTO LIGHTRAG_GRAPH_EDGES a
                ...
                WHEN NOT MATCHED THEN
                    INSERT(workspace,source_name,target_name,weight,keywords,description,source_chunk_id,content,content_vector,file_path)
                    values (:workspace,:source_name,:target_name,:weight,:keywords,:description,:source_chunk_id,:content,:content_vector,:file_path)
                WHEN MATCHED THEN
                    UPDATE SET
                    weight=:weight,keywords=:keywords,description=:description,
                    source_chunk_id=:source_chunk_id,content=:content,content_vector=:content_vector,
                    updatetime=SYSDATE,file_path=:file_path""",
    ...
    "get_matching_nodes": """
                ...
                """,
    "get_all_nodes": """
                SELECT name, name as entity_type, description, source_chunk_id 
                FROM LIGHTRAG_GRAPH_NODES
                WHERE workspace = :workspace
                ORDER BY name
                fetch first :max_nodes rows only
                """,
    "get_related_edges": """
                SELECT source_name, target_name, weight, keywords, description, source_chunk_id
                FROM LIGHTRAG_GRAPH_EDGES
                WHERE workspace = :workspace
                AND (source_name IN (:node_names) or target_name in (:node_names))
                """,
