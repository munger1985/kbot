...
class BaseModelPool(ABC, Generic[T]):
    ...
    def set_session_factory(self, session_factory: Callable) -> None:
        """Bind the App-owned DB Session Factory before model loading."""
        if self._models:
            raise RuntimeError("cannot replace the database factory after models are loaded")
        self._session_factory = session_factory

    @property
    def oracle_session(self):
        """Get a session from the owning App runtime."""
        if self._session_factory is None:
            raise RuntimeError("model pool database session factory is not configured")
