...
class AssistantInitializerTest(unittest.TestCase):
    ...
    def test_bootstrap_contains_only_assistant_initial_resources(self) -> None:
        ...
        self.assertIn("KBOT_ASST_MODEL_BINDING", source)
        self.assertIn("KBOT_ASST_RUN", source)
        self.assertIn("KBOT_ASST_MEDIA_ASSET", source)
        self.assertIn("缺表：", source)
        self.assertIn("LISTAGG", upper_source)
