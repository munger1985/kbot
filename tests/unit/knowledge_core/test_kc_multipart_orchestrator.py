...
class FakeIntake:
    ...
    async def abort(self, command):
        self.calls.append(("abort", command.failure_code))
...
class MultipartOrchestratorTest(unittest.TestCase):
    ...
    def test_failure_deletes_objects_and_compensates_database_reservation(self):
        """最终事务失败时同时补偿对象文件和数据库预留身份。"""
        payload = b"abc"
        data = {
            "bundle": {
                "source_id": "A-2",
        ...
        class FailingIntake(FakeIntake):
            async def accept_published(self, command):
                self.calls.append("accept")
                raise RuntimeError("最终事务失败")

        with tempfile.TemporaryDirectory() as directory:
            source = Path(directory) / "a.txt"
            source.write_bytes(payload)
            intake, store = FailingIntake(), FakeStore()
            with self.assertRaisesRegex(RuntimeError, "最终事务失败"):
                asyncio.run(
                    KnowledgeCoreMultipartOrchestrator(
                        intake_service=intake,
                        object_store=store,
                    ).accept(
                        MultipartIntakeCommand(
                            1,
                            "assets",
                            "svc:portal",
                            "key-failed",
                            KmAssetIntakeManifest.model_validate(data),
                            {"attachment_0": source},
                        )
                    )
                )

        self.assertIn(("abort", "RuntimeError"), intake.calls)
        self.assertEqual(
            4,
            len([call for call in store.calls if call[0] == "delete"]),
        )
