...
from scripts.check_aiops_entity_schema import AIOPS_ENTITY_CLASSES
from scripts.check_oracle_schema import SERVICE_TABLES
    ...
    for table_name, body in re.findall(
        ...
        columns = {
            ...
            if (
                match := re.match(
                    r"\s{4}([A-Z][A-Z0-9_]*)\s+"
                    r"(?:RAW|NUMBER|VARCHAR2|CLOB|TIMESTAMP)\b",
                    line,
                )
            )
        }
