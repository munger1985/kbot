/* 知识检索公共 API Client：附加 Authorization，不保存下游服务凭据。 */
(function () {
  "use strict";
  const basePath = "/api/v1/apps/knowledge-retrieval";

  function requestId() {
    return globalThis.KBotKnowledgeAuth?.uuid?.()
  ...
  function redirectIfUnauthorized() {
    globalThis.KBotKnowledgeAuth?.clear?.();
  ...
  async function request(path, options = {}) {
    ...
    if (!response.ok) {
      ...
      error.code = payload?.code || payload?.detail?.code || "KNOWLEDGE_RETRIEVAL_REQUEST_FAILED";
  ...
  async function stream(path, handlers = {}, signal) {
    ...
    if (!response.ok || !response.body) {
      ...
      const error = new Error(errorMessage(payload, response.status));
      error.status = response.status;
      error.code = payload?.code || payload?.detail?.code || "KNOWLEDGE_RETRIEVAL_EVENT_STREAM_FAILED";
      throw error;
    }
  ...
  function isGrokModel(row) {
    const name = String(row?.provider_model_name ?? "").trim().toLowerCase();
    return name.split(/[./:_]+/).some((part) => part === "grok" || part.startsWith("grok-"));
  }

  globalThis.KBotKnowledgeApi = {
    basePath, json, request, requestBlob, requestId, stream, withQuery,
    ModelCategory, items, modelCategory, isActiveModel, isGrokModel,
  };
})();
