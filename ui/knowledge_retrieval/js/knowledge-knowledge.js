/* 知识检索知识问答：复用 Agent Runtime 会话、Turn、事件和引用能力。 */
(function () {
  "use strict";

  const { escapeHtml, toast } = KBotKnowledgeShell;
  ...
  async function loadAgents() {
    ...
    const payload = await KBotKnowledgeApi.request("/agents");
    state.agents = KBotKnowledgeApi.items(payload).filter((row) => row.status === "ACTIVE");
  ...
  function renderConversationList() {
    ...
    if (!state.conversations.length) {
      list.innerHTML = '<li class="knowledge-chat-empty">当前 Agent 暂无会话</li>';
      return;
    }
    list.innerHTML = state.conversations.map((row) => `
      <li class="knowledge-conversation-row">
        <button class="knowledge-conversation-select" type="button" data-conversation-id="${escapeHtml(row.conversation_id)}" aria-current="${String(state.active?.conversation_id) === String(row.conversation_id)}">
          <strong>${escapeHtml(row.title || "未命名会话")}</strong>
          <small>${escapeHtml(formatDate(row.last_active_at))}</small>
        </button>
        <button class="knowledge-conversation-delete small" type="button" data-delete-conversation="${escapeHtml(row.conversation_id)}" aria-label="删除会话 ${escapeHtml(row.title || "未命名会话")}">删除</button>
      </li>`).join("");
  }
  ...
  function renderEmptyChat() {
    ...
    $("knowledge-chat-stream").innerHTML = `<div class="knowledge-chat-empty">${selectedAgent() ? "新建会话后即可开始问文、问数或普通对话。" : "当前没有可用 Agent。"}</div>`;
  ...
  async function loadConversations(selectId) {
    ...
    const rows = KBotKnowledgeApi.items(await KBotKnowledgeApi.request("/conversations?limit=50"));
  ...
  async function createConversation() {
    ...
    try {
      const created = await KBotKnowledgeApi.json("/conversations", "POST", {
  ...
  async function selectConversation(row) {
    state.active = await KBotKnowledgeApi.request(`/conversations/${row.conversation_id}`);
    renderConversationList();
  ...
  async function deleteConversation(row) {
    ...
    await KBotKnowledgeApi.request(`/conversations/${row.conversation_id}?expected_row_version=${row.row_version}`, { method: "DELETE" });
  ...
  function messageMarkup(role, label, text, runId) {
    const content = role === "knowledge" ? renderMarkdown(text) : escapeHtml(text);
    const run = runId ? ` data-run-id="${escapeHtml(runId)}"` : "";
    const references = runId ? `<div class="knowledge-message-references" data-references-for="${escapeHtml(runId)}"></div>` : "";
    return `<div class="knowledge-message ${role}"${run}><div class="knowledge-message-meta">${escapeHtml(label)}</div><div class="knowledge-message-content">${content}</div>${references}</div>`;
  }
  ...
  function renderReferences(runId, source) {
    ...
    host.innerHTML = rows.map((row, index) => {
      ...
      return `<button type="button" class="knowledge-reference" data-run-id="${escapeHtml(runId)}" data-reference-label="${escapeHtml(label)}">${escapeHtml(label)} · ${escapeHtml(title)}</button>`;
    }).join("");
  }

  async function loadTurns() {
    if (!state.active) return renderEmptyChat();
    const payload = await KBotKnowledgeApi.request(`/conversations/${state.active.conversation_id}/turns?after=0&limit=200`);
    const turns = Array.isArray(payload?.turns) ? payload.turns : KBotKnowledgeApi.items(payload);
    const stream = $("knowledge-chat-stream");
    if (!turns.length) {
      stream.innerHTML = '<div class="knowledge-chat-empty">这是一个新会话，请输入问题。</div>';
      return;
    }
    stream.innerHTML = turns.map((turn) => {
      const user = contentText(turn.user_item);
      const knowledge = contentText(turn.knowledge_item);
      const waiting = turn.status && turn.status !== "COMPLETED" ? `处理状态：${turn.status}` : "";
      return `${user ? messageMarkup("user", "你", user) : ""}${knowledge ? messageMarkup("knowledge", agentName(state.active.agent_id), knowledge, turn.run_id) : (waiting ? messageMarkup("knowledge", agentName(state.active.agent_id), waiting, turn.run_id) : "")}`;
    }).join("");
    turns.forEach((turn) => {
      const references = turn.knowledge_item?.content?.references;
  ...
  function appendPending(input) {
    const stream = $("knowledge-chat-stream");
    if (stream.querySelector(".knowledge-chat-empty")) stream.innerHTML = "";
    stream.insertAdjacentHTML("beforeend", messageMarkup("user", "你", input));
    const wrapper = document.createElement("div");
    wrapper.className = "knowledge-message knowledge pending";
    wrapper.setAttribute("aria-busy", "true");
    wrapper.innerHTML = `<div class="knowledge-message-meta">${escapeHtml(agentName(selectedAgentId()))}</div><div class="knowledge-message-content" aria-live="polite">正在分析问题并选择问文、问数或普通对话路径…</div><div class="knowledge-message-references"></div>`;
    stream.append(wrapper);
    stream.scrollTop = stream.scrollHeight;
    return {
      wrapper,
      content: wrapper.querySelector(".knowledge-message-content"),
      references: wrapper.querySelector(".knowledge-message-references"),
      answer: "",
  ...
  async function createTurn(input, idempotencyKey) {
    ...
    const payload = {
      ...
      client_metadata: { source: "knowledge-ui" },
    };
    try {
      return await KBotKnowledgeApi.json(path, "POST", payload, options);
    } catch (error) {
      if (!isRetryable(error)) throw error;
      $("knowledge-progress").textContent = "连接中断，正在确认已提交的 Turn";
      await new Promise((resolve) => window.setTimeout(resolve, 300));
      return KBotKnowledgeApi.json(path, "POST", payload, options);
    }
  }

  async function refreshActive() {
    if (state.active) state.active = await KBotKnowledgeApi.request(`/conversations/${state.active.conversation_id}`);
  }
  ...
  async function send(event) {
    ...
    try {
      const receipt = await createTurn(input, KBotKnowledgeApi.requestId());
      if (receipt.run_id) {
        ...
        await KBotKnowledgeApi.stream(`/runs/${encodeURIComponent(receipt.run_id)}/events`, {
          lastEventId: receipt.event_cursor,
          onEvent: (item) => applyRunEvent(pending, item),
        });
        const run = await KBotKnowledgeApi.request(`/runs/${receipt.run_id}`);
        if (run.status !== "COMPLETED") {
          const error = new Error(run.error_message || `Run 执行结束但状态为 ${run.status}`);
          error.code = run.error_code || "KNOWLEDGE_RETRIEVAL_RUN_NOT_COMPLETED";
          throw error;
        }
        if (!pending.answer || !pending.referencesReceived) {
          const result = await KBotKnowledgeApi.request(`/runs/${receipt.run_id}/result`);
          if (!pending.answer) {
  ...
  async function resultForRun(runId) {
    ...
    if (!result) {
      result = await KBotKnowledgeApi.request(`/runs/${runId}/result`);
  ...
  async function openReference(runId, label) {
    ...
    const preview = await KBotKnowledgeApi.request(`/runs/${runId}/references/${encodeURIComponent(label)}/preview`);
  ...
  async function openReferenceContent(button) {
    ...
    try {
      const blob = await KBotKnowledgeApi.requestBlob(button.dataset.openReferenceContent);
