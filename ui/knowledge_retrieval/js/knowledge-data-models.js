...
(function () {
  ...
  const items = (payload) => KBotKnowledgeApi.items(payload);
  ...
  function invalidateConnectionTest() {
    ...
    node.className = "knowledge-notice warn";
  ...
  async function testConnection() {
    ...
    try {
      const result = await KBotKnowledgeApi.json(
      ...
      const node = element("connection-test-status");
      node.className = "knowledge-notice";
  ...
  async function createSource(event) {
    ...
    const created = await KBotKnowledgeApi.json(
    ...
    testStatus.className = "knowledge-notice warn";
  ...
  function renderSources() {
    ...
    if (!state.sources.length) {
      node.innerHTML = '<div class="knowledge-empty"><div><strong>尚无数据库连接</strong><p>配置只读连接并完成测试后，才能发现表和建立问数模型。</p></div></div>';
  ...
  async function loadSources() {
    const payload = await KBotKnowledgeApi.json(
  ...
  async function loadSnapshot(snapshotId, pollToken) {
    const snapshot = await KBotKnowledgeApi.json(
  ...
  function llmModels() {
    return state.catalog.filter((row) => (
      KBotKnowledgeApi.isActiveModel(row)
      && KBotKnowledgeApi.modelCategory(row) === KBotKnowledgeApi.ModelCategory.LLM
    ));
  }
  ...
  async function generateModel(event) {
    ...
    const receipt = await KBotKnowledgeApi.json(
  ...
  async function waitForGeneration(jobId) {
    for (let attempt = 0; attempt < 80; attempt += 1) {
      const job = await KBotKnowledgeApi.json(
  ...
  async function loadModels() {
    const payload = await KBotKnowledgeApi.json("/data-models?limit=200", "GET");
    state.models = items(payload);
    const details = await Promise.all(state.models.map(async (row) => {
      try {
        return await KBotKnowledgeApi.json(`/data-models/${encodeURIComponent(row.semantic_model_id)}`, "GET");
      } catch (_) {
        return row;
      }
    }));
  ...
  async function openSemanticModel(modelId) {
    const detail = await KBotKnowledgeApi.json(`/data-models/${encodeURIComponent(modelId)}`, "GET");
    ...
    element("validation-status").className = "knowledge-badge";
  ...
  function renderValidation(result) {
    ...
    if (!rows.length) {
      node.innerHTML = `<div class="knowledge-notice ${result.error_code ? "warn" : ""}">${escapeHtml(result.error_code || "验证完成，但结果为空。")}</div>`;
      return;
    }
  ...
  async function runValidation() {
    ...
    element("validation-status").className = "knowledge-badge warn";
    const receipt = await KBotKnowledgeApi.json(
    ...
    for (let attempt = 0; attempt < 80; attempt += 1) {
      ...
      if (terminalValidationStatuses.has(result.status)) {
        ...
        element("validation-status").className = `knowledge-badge ${successful ? "good" : "bad"}`;
  ...
  async function loadReferenceData() {
    const [connectors, catalog] = await Promise.all([
      KBotKnowledgeApi.json("/data-models/connector-capabilities", "GET"),
      KBotKnowledgeApi.json("/model-catalog", "GET"),
    ]);
  ...
  KBotKnowledgeShell.ready.then(async (access) => {
    ...
    try {
      ...
      showError(error);
      element("data-source-list").innerHTML = '<div class="knowledge-empty"><div><strong>无法读取问数配置</strong><p>请确认 Data Query API 与 Worker 均已启动。</p></div></div>';
