...
(function () {
  ...
  function renderProcessing(items) {
    ...
    node.innerHTML = items.map((item) => {
      ...
      return `<tr>
        ...
        <td><div class="knowledge-row-actions">${actions}</div></td>
      </tr>`;
    }).join("");
  ...
  function renderApprovals(items) {
    ...
    node.innerHTML = items.map((item) => {
      ...
      return `<tr>
        ...
        <td><div class="knowledge-row-actions">
  ...
  async function loadApprovals() {
    ...
    const payload = await KBotKnowledgeApi.json(`/knowledge-cores/${selectedCollectionId}/approvals`, "GET");
  ...
  async function loadProcessing() {
    ...
    const [payload] = await Promise.all([
      KBotKnowledgeApi.json(`/knowledge-cores/${row.collection_id}/processing`, "GET"),
      loadApprovals(),
    ]);
  ...
  function modelsByCategory(category) {
    return catalog.filter((row) => KBotKnowledgeApi.modelCategory(row) === category);
  }
  ...
  function resetForm() {
    ...
    const textEmbeddings = modelsByCategory(KBotKnowledgeApi.ModelCategory.TXT_EMBEDDING);
    fillSelect(
    ...
    fillSelect(
      ...
      modelsByCategory(KBotKnowledgeApi.ModelCategory.IMG_EMBEDDING),
      "",
      "不绑定",
    );
  }
  ...
  function fillForm(row) {
    ...
    fillSelect(
      document.getElementById("kc-embedding"),
      modelsByCategory(KBotKnowledgeApi.ModelCategory.TXT_EMBEDDING),
      models.embedding,
      "选择文本 Embedding",
    );
    fillSelect(
  ...
  function actionButtons(row) {
    ...
    if (row.status === "DELETING") {
      return '<span class="knowledge-badge warn">清理中</span>';
    }
    const buttons = [
  ...
  async function loadPage() {
    const [listed, catalogRows] = await Promise.all([
      KBotKnowledgeApi.json("/knowledge-cores", "GET"),
      KBotKnowledgeApi.json("/model-catalog", "GET"),
    ]);
    rows = collectionItems(listed);
    catalog = KBotKnowledgeApi.items(catalogRows).filter((row) => KBotKnowledgeApi.isActiveModel(row));
  ...
  async function changeStatus(row, status) {
    await KBotKnowledgeApi.json(`/knowledge-cores/${row.collection_id}`, "PATCH", {
  ...
  async function handleAction(action, collectionId) {
    ...
    if (action === "delete") {
      ...
      if (!confirmed) return;
      await KBotKnowledgeApi.json(`/knowledge-cores/${row.collection_id}`, "DELETE");
      await loadPage();
  ...
  async function submitForm(event) {
    ...
    try {
      if (editingId == null) {
        await KBotKnowledgeApi.json("/knowledge-cores", "POST", payload);
      ...
      await KBotKnowledgeApi.json(`/knowledge-cores/${editingId}`, "PATCH", update);
      closeDialog("kc-dialog");
      resetForm();
      await loadPage();
  ...
  async function uploadFiles(event) {
    ...
    try {
      await KBotKnowledgeApi.request(`/knowledge-cores/${selectedCollectionId}/ingestions/user-files`, {
        method: "POST",
        body: form,
