/* 多媒体创作工作台会话：登录、改密与本地 Session，不保存下游服务凭据。 */
(function () {
  "use strict";

  const storageKey = "kbot.media.session.v1";

  function baseUrl() {
    const value = String(globalThis.KBOT_UI_CONFIG?.mainApiBaseUrl || "").trim().replace(/\/+$/, "");
    if (!value) throw new Error("多媒体创作工作台未加载 Main API 部署配置，请刷新页面");
    return value;
  }
  ...
  async function raw(path, options = {}, token = "") {
    ...
    if (!response.ok) {
      ...
      error.code = payload?.code || payload?.detail?.code || "MEDIA_STUDIO_AUTH_FAILED";
  ...
  async function login(body) {
    return save(await raw("/api/v1/apps/media-studio/auth/login", {
      method: "POST",
  ...
  async function changePassword(body) {
    const session = load();
    if (!session?.access_token) throw new Error("请先登录多媒体创作工作台");
    return save({
      ...session,
      ...(await raw("/api/v1/apps/media-studio/auth/password", {
        method: "POST",
        body: JSON.stringify(body),
