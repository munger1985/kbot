...
(function () {
  ...
  const { badge, capability, capabilityLabel, escapeHtml, toast } = KBotMediaShell;
  ...
  function paintCapability() {
    ...
    node.className = `media-badge ${tone}`;
  ...
  function empty(title, copy) {
    return `<div class="media-empty"><div><strong>${escapeHtml(title)}</strong><p>${escapeHtml(copy)}</p></div></div>`;
  }
  ...
  function renderHistory(rows) {
    ...
    node.innerHTML = rows.map((row) => `
      <article class="media-history-item" data-run-id="${escapeHtml(row.run_id)}" ${row.run_id === currentRunId ? 'aria-current="true"' : ""}>
        <button class="media-history-open" type="button" data-run-id="${escapeHtml(row.run_id)}">
        ...
        <button class="small danger media-history-delete" type="button" data-run-id="${escapeHtml(row.run_id)}">删除</button>
      </article>`).join("");
    node.querySelectorAll(".media-history-open").forEach((button) => {
      button.addEventListener("click", () => openRun(button.dataset.runId));
    });
    node.querySelectorAll(".media-history-delete").forEach((button) => {
  ...
  async function renderPreview(run) {
    ...
    for (const assetId of assetIds) {
      const blob = await KBotMediaApi.requestBlob(`/media-assets/${assetId}/content`);
  ...
  async function refreshHistory() {
    const rows = await KBotMediaApi.json("/image-generations/runs", "GET");
    renderHistory(Array.isArray(rows) ? rows : []);
  }

  async function loadRun(runId) {
    const [run, events] = await Promise.all([
      KBotMediaApi.json(`/image-generations/runs/${runId}`, "GET"),
      KBotMediaApi.json(`/image-generations/runs/${runId}/events`, "GET"),
    ]);
    fillComposer(run);
  ...
  async function deleteRun(runId) {
    ...
    if (!confirmed) return;
    try {
      await KBotMediaApi.json(`/image-generations/runs/${runId}`, "DELETE");
  ...
  KBotMediaShell.ready.then(async (access) => {
    ...
    document.getElementById("image-form")?.addEventListener("submit", async (event) => {
      ...
      try {
        const run = await KBotMediaApi.json("/image-generations/runs", "POST", payload, {
          headers: { "Idempotency-Key": KBotMediaApi.requestId() },
        });
