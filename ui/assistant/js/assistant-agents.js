...
(function () {
  ...
  function emptyRow(title, copy) {
    return `<tr><td colspan="6"><div class="assistant-empty"><div><strong>${escapeHtml(title)}</strong><p>${escapeHtml(copy)}</p></div></div></td></tr>`;
  }
  ...
  function fillResourceOptions(current) {
    ...
    const dataModels = state.options.data_models.map((row) => {
      ...
      const reason = row.selectable ? "" : row.active_version == null ? " · 未发布" : " · 缺少有效策略";
  ...
  async function submitForm(event) {
    ...
    try {
      ...
      if (!state.editing) {
        await KBotAssistantApi.json("/agents", "POST", {
        ...
        closeDialog();
        await loadAll();
        toast("Agent 草稿已创建。如需问数，请为当前版本创建查询 Binding。");
        return;
      }

      const changes = versionChanges(version, state.editing);
      let targetStatus = element("agent-status").value;
      if (Object.keys(changes).length && version.data_model_ids.length && targetStatus === "ACTIVE") {
        targetStatus = "DRAFT";
        toast("运行版本已修改，Agent 已转为草稿；请重新创建查询 Binding 后再启用。");
      }
      const payload = {
      ...
      await KBotAssistantApi.json(`/agents/${encodeURIComponent(state.editing.agent_id)}`, "PATCH", payload);
      closeDialog();
      await loadAll();
      toast("Agent 已保存。");
    } finally {
  ...
  async function handleAction(action, agentId) {
    ...
    if (!row) return;
    if (action === "edit") {
      const detail = await KBotAssistantApi.json(`/agents/${encodeURIComponent(agentId)}`, "GET");
      fillForm(detail);
      openDialog();
      return;
    }
    ...
    if (action === "archive") {
      ...
      await KBotAssistantApi.json(
        KBotAssistantApi.withQuery(`/agents/${encodeURIComponent(agentId)}`, { expected_row_version: row.row_version }),
        "DELETE",
      );
      await loadAll();
      toast("Agent 已删除并归档。");
  ...
  async function loadAll() {
    ...
    try {
      const [rows, options] = await Promise.all([
        KBotAssistantApi.json("/agents", "GET"),
        KBotAssistantApi.json("/agents/options", "GET"),
      ]);
      state.rows = Array.isArray(rows) ? rows : KBotAssistantApi.items(rows);
      state.options = {
