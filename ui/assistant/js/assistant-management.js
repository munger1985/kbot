...
(function () {
  ...
  function roleLabel(role) {
    if (role === "X_SEARCH") return "X 实时搜索";
    if (role === "IMAGE_GENERATION") return "文生图";
    return "知识回答";
  }

  function roleRequirement(role) {
    if (role === "X_SEARCH") return "X Search、引用、流式响应";
    if (role === "IMAGE_GENERATION") return "图像生成、产物返回";
    return "文本回答";
  }
  ...
  function renderBindings() {
    ...
    if (!node) return;
    const roles = ["KNOWLEDGE", "X_SEARCH", "IMAGE_GENERATION"];
    node.innerHTML = roles.map((role) => {
      ...
      return `<tr>
        ...
        <td>${status}</td>
        <td>${escapeHtml(row?.capability_verified_at || "—")}</td>
      </tr>`;
    }).join("");
  }
  ...
  async function loadBindingsPage() {
    ...
    fillSelect(document.getElementById("knowledge-model"), knowledge, currentBinding("KNOWLEDGE")?.model_id);
    fillSelect(document.getElementById("search-model"), search, currentBinding("X_SEARCH")?.model_id);
  ...
  async function saveBindings(event) {
    ...
    try {
      for (const [role, id] of entries) {
        ...
        await KBotAssistantApi.json("/bindings", "PUT", payload);
        saved += 1;
      }
      if (!saved) {
  ...
  async function loadAssets() {
    ...
    const query = status ? KBotAssistantApi.withQuery("/media-assets", { status }) : "/media-assets";
    const rows = await KBotAssistantApi.json(query, "GET");
    const now = Date.now();
  ...
  async function loadRuns() {
    ...
    const query = KBotAssistantApi.withQuery("/runs", { kind, status });
    const rows = await KBotAssistantApi.json(query, "GET");
    const filtered = (Array.isArray(rows) ? rows : []).filter((row) => {
