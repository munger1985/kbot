...
(function () {
  ...
  const PORTAL_NAME = "assistant_portal";
  let rows = [];
  let editingId = null;
  ...
  function actionButtons(row) {
    ...
    const buttons = [];
    if (isCurrent(row)) {
      buttons.push('<span class="assistant-badge good">当前</span>');
  ...
  async function loadRows() {
    const payload = await KBotAssistantApi.json("/domains", "GET");
  ...
  async function switchDomain(row) {
    ...
    const next = await KBotAssistantApi.json("/api/v1/auth/switch-domain", "POST", {
      domain_id: Number(row.domain_id),
    });
    KBotAssistantAuth.save({ ...current, ...next });
    location.reload();
  }
  ...
  async function handleAction(action, domainId) {
    ...
    if (action === "enable") {
      await KBotAssistantApi.json(`/domains/${row.domain_id}`, "PATCH", {
    ...
    if (action === "delete") {
      ...
      await KBotAssistantApi.json(
        KBotAssistantApi.withQuery(`/domains/${row.domain_id}`, {
          expected_row_version: row.row_version,
        }),
        "DELETE",
      );
  ...
  async function submitForm(event) {
    ...
    try {
      if (editingId == null) {
        await KBotAssistantApi.json("/domains", "POST", {
          name,
      ...
      await KBotAssistantApi.json(`/domains/${editingId}`, "PATCH", payload);
      closeDialog("domain-dialog");
      resetForm();
      await loadRows();
      toast("Domain 已保存。");
