...
`api.test_auth_bypass_enabled = true`，并将该 Origin 加入 Main API 的
`allowed_origins`。页面会发送 `X-KBot-Test-Auth: true`，因此不需要 Portal API
Key；仍需填写数据库中有效的 Domain ID 和测试 User ID。

- `knowledge-core.html`：创建 Domain、创建/选择 Collection；一个文件可直接作为单文件 Bundle
  入库；多个文件可选择分别入库，或共同组成一个 Bundle。上传记录会从计算 Hash
  开始自动跟踪审批、解析、索引和最终可检索状态，并保存在当前浏览器中；还可手工
  查询 Bundle / Revision 状态和绑定 Agent。页面不读取 MetaDB，也不构造 Asset。
