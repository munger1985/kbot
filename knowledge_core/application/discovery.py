...
class KnowledgeCoreProfileService:
    ...
    async def claim(self, claim: ParseTaskClaim) -> list[ClaimedProfileTask]:
        claim.validate()
        now = datetime.now(timezone.utc)
        async with self._uow_factory() as uow:
            if uow.jobs is None or uow.session is None:
                raise RuntimeError("Knowledge Core Unit of Work is not initialized")
            jobs = await uow.jobs.claim_profile_candidates(now=now, limit=claim.max_tasks)
            tasks: list[ClaimedProfileTask] = []
            for job in jobs:
                lease_until = claim_job(job, claim, now)
                tasks.append(ClaimedProfileTask(
                    job_id=job.ingestion_job_id, worker_id=claim.worker_id,
                    input_fingerprint=job.input_fingerprint, lease_until=lease_until,
                ))
            if tasks:
