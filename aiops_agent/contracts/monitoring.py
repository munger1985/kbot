...
from aiops_agent.domain.monitoring.events import (
    MonitorEventStatus,
    MonitorSeverity,
)
...
class NormalizedMonitorEvent(BaseModel):
    ...
    event_status: MonitorEventStatus
...
class NormalizedWebhookBatch(BaseModel):
    model_config = ConfigDict(extra="forbid", frozen=True)

    provider_delivery_id: str | None = Field(default=None, max_length=256)
    events: tuple[NormalizedMonitorEvent, ...] = ()
    warnings: tuple[str, ...] = ()
