...
def create_aiops_scheduler_probe(
    ...
    """创建尚不领取 Plan 的 Scheduler 步骤 0 探针。"""
    ...
    async def lifespan(app):
        ...
        await runtime.start()
        logger.info("正在启动 AIOps Scheduler 步骤 0 探针")
        try:
            yield
        finally:
            await runtime.close()
            logger.info("AIOps Scheduler 探针资源已释放")

    return create_process_app(
        title="KBot AIOps Scheduler Probe",
        description="AIOps Scheduler 的内部存活与就绪探针。",
        service_name=config.service_name,
