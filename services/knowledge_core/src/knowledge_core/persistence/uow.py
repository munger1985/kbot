...
class KnowledgeCoreUnitOfWork:
    ...
    def __init__(
        ...
        job_wakeup_publisher: JobWakeupPublisher | None = None,
    ):
        self._session_factory = session_factory
        self._job_wakeup_publisher = job_wakeup_publisher
        self._pending_wakeup_channels: set[str] = set()
    ...
    async def __aenter__(self) -> "KnowledgeCoreUnitOfWork":
        ...
        self.jobs = IngestionJobRepository(
            self.session,
            on_job_added=self._track_pending_job,
        )
    ...
    async def commit(self) -> None:
        ...
        if (
            self._job_wakeup_publisher is not None
            and self._pending_wakeup_channels
        ):
            try:
                async with self.session.begin_nested():
                    await self._job_wakeup_publisher.signal(
                        self.session,
                        frozenset(self._pending_wakeup_channels),
                    )
            except Exception as exc:
                # 通知仅用于降低空轮询，失败不能阻断持久任务提交。
                logger.warning(
                    "KC 任务唤醒信号发送失败，任务将由兜底扫描发现：{}",
                    exc,
                )
    ...
    def _track_pending_job(self, job_type: str) -> None:
        self._pending_wakeup_channels.add(
...
def create_kc_uow(
    ...
    return KnowledgeCoreUnitOfWork(
        session_factory,
