    ...
    async def delete_empty_unreferenced(self, *, bundle_id: UUID) -> None:
        await self.session.execute(
            delete(KcBundleEntity).where(
                KcBundleEntity.bundle_id == bundle_id,
                KcBundleEntity.availability_status == "EMPTY",
                KcBundleEntity.current_revision_id.is_(None),
                ~exists(
                    select(1).where(
...
class DocumentRepository:
    ...
    async def add(self, entity: KcDocumentEntity) -> KcDocumentEntity:
        self.session.add(entity); await self.session.flush(); return entity
    async def delete_reserved_unreferenced(
        self, *, bundle_id: UUID, document_ids: list[UUID],
    ) -> None:
        if not document_ids:
            return
        await self.session.execute(
            delete(KcDocumentEntity).where(
                KcDocumentEntity.bundle_id == bundle_id,
                KcDocumentEntity.document_id.in_(document_ids),
                ~exists(
                    select(1).where(
                        KcDocumentVersionEntity.document_id
                        == KcDocumentEntity.document_id
                    )
                ),
                ~exists(
                    select(1).where(
                        KcBundleRevisionDocumentEntity.document_id
                        == KcDocumentEntity.document_id
                    )
                ),
            )
...
class IngestionJobRepository:
    ...
    def __init__(
        ...
        on_job_added: Callable[[str], None] | None = None,
    ):
        self.session = session
        self._on_job_added = on_job_added

    async def _claim_by_types(
        self,
        *,
        job_types: tuple[str, ...],
        now: datetime,
        ...
        if limit <= 0 or not job_types:
            return []

        eligibility = (
            KcIngestionJobEntity.job_type.in_(job_types),
            or_(
                and_(
                    KcIngestionJobEntity.job_status.in_(
                        ("PENDING", "RETRY_WAIT")
                    ),
                    KcIngestionJobEntity.available_at <= now,
                ),
                and_(
                    KcIngestionJobEntity.job_status == "RUNNING",
                    KcIngestionJobEntity.lease_until.is_not(None),
                    KcIngestionJobEntity.lease_until <= now,
                ),
            ),
        )
        type_priority = case(
            ...
            value=KcIngestionJobEntity.job_type,
            else_=len(job_types),
        )
        scan_limit = max(
    ...
    async def claim_candidates_by_types(
        self,
        *,
        job_types: tuple[str, ...],
        now: datetime,
        ...
        """按调用方声明的类型优先级统一抢占异步任务。"""
        return await self._claim_by_types(
            job_types=job_types,
            now=now,
            limit=limit,
        )
...
class EvidenceRepository:
    ...
    def _to_hit(
        ...
        return EvidenceHit(
            ...
            locator=entity.locator_json,
            locator_schema_version=entity.locator_schema_version,
            source_spans=tuple(entity.source_spans_json or []),
