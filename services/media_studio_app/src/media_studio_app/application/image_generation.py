...
from media_studio_app.application.catalog import is_role_ready, load_catalog_model, safe_model_snapshot
from media_studio_app.application.errors import MediaStudioApplicationError
from media_studio_app.application.runtime import IMAGE_KIND, append_event, event_view, not_found, run_view
from media_studio_app.entities import MediaStudioPromptRevisionEntity, MediaStudioRunEntity
...
class ImageGenerationService:
    ...
    async def create(self, command: CreateImageGenerationCommand) -> tuple[dict[str, Any], bool]:
        ...
        revision = MediaStudioPromptRevisionEntity(
            prompt_revision_id=revision_id,
            domain_id=command.domain_id,
            run_id=run_id,
            actor_id=command.actor_id,
