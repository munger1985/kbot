...
from media_studio_app.api import (
    binding_router,
...
async def lifespan(app: FastAPIOffline):
    LogManager(LogConfig(
        service="media_studio_app", process="api", log_dir=settings.log.dir,
        level=settings.log.level, rotation=settings.log.rotation,
        retention=settings.log.retention,
    )).setup()
    database_runtime = create_database_runtime(settings)
    uow_factory = create_media_studio_app_uow(database_runtime.session_factory)
    ...
    app.state.run_query_service = MediaStudioRunQueryService(uow_factory=uow_factory)
...
app = FastAPIOffline(
    title="KBot Media Studio Internal API",
...
app.middleware("http")(
    create_scoped_internal_auth_middleware(
        ...
        allowed_callers={
            "kbot-main-api": frozenset({"media_studio.manage"}),
            "kbot-model-embedding": frozenset({"media_studio.manage"}),
            "kbot-model-llm": frozenset({"media_studio.manage"}),
            "kbot-model-visual": frozenset({"media_studio.manage"}),
            "kbot-model-vlm": frozenset({"media_studio.manage"}),
            "kbot-model-ocr": frozenset({"media_studio.manage"}),
        },
    )
)
