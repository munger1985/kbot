...
from knowledge_retrieval_app.application import KnowledgeRetrievalResearchWorker
...
async def main() -> None:
    ...
    LogManager(LogConfig(
        service="knowledge_retrieval_app", process="worker", log_dir=settings.log.dir,
        level=settings.log.level, rotation=settings.log.rotation, retention=settings.log.retention,
    )).setup()
    runtime = create_database_runtime(settings)
    worker = KnowledgeRetrievalResearchWorker(
        uow_factory=create_knowledge_retrieval_app_uow(runtime.session_factory),
