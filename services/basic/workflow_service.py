# services/basic/workflow_service.py — SOP 工作流服务（Oracle 适配版）
...
class WorkflowService:
    ...
    async def add_workflow(
        ...
        async with self.db_session as session:
            ...
            try:
                await repo.create_workflow(new_workflow)
    ...
    async def update_workflow(
        ...
        async with self.db_session as session:
            ...
            await repo.update_workflow(workflow_id, update_data)
    ...
    async def update_metadata(
        ...
        async with self.db_session as session:
            repo = WorkflowRepository(session)
            updated_entity = await repo.update_workflow(workflow_id, update_data)
            ...
            if any(field in update_data for field in search_fields):
                ...
                await repo.update_workflow(workflow_id, {"embedding": vector})
    ...
    async def search_workflow(self, agent_id: str, query: str, top_k: int = 1) -> list[dict[str, Any]]:
        """语义搜索：Oracle 23ai 向量检索"""
        try:
            vector = await self._get_embedding(agent_id, query)
            async with self.db_session as session:
                repo = WorkflowRepository(session)
                hits = await repo.search_workflow(
                    agent_id=agent_id,
                    query_text=query,
                    query_vector=vector,
                    top_k=top_k
    ...
    async def _get_embedding(self, agent_id: str, text: str) -> list[float]:
        """获取文本的向量表示"""
    ...
    async def get_workflows_by_agent(self, agent_id: str, is_active: bool | None = None) -> list[dict[str, Any]]:
        """获取某个 Agent 下所有流程"""
        async with self.db_session as session:
            repo = WorkflowRepository(session)
