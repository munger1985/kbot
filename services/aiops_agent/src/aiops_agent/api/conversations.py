...
async def create_or_append(payload: MessagePayload, request: Request, context: AuthContext = Depends(get_aiops_auth_context)):
    ...
    conversation = await service.create_or_append(domain_id=domain_id, agent_id=payload.agent_id, actor_id=actor_id, message=payload.message, conversation_id=payload.conversation_id)
    ...
    receipt = await request.app.state.aiops_runtime_service.create_run(CreateOpsRunCommand(
        ...
        client_metadata={"trace_id": context.trace_id, "conversation_id": conversation["conversation_id"], "report_requested": payload.request_report},
    ))
...
async def upload_evidence(conversation_id: UUID, request_id: UUID, payload: EvidenceUploadPayload, request: Request, context: AuthContext = Depends(get_aiops_auth_context)):
    domain_id, actor_id = _scope(request, context)
    return await request.app.state.conversation_service.upload_evidence_file(domain_id=domain_id, conversation_id=conversation_id, request_id=request_id, actor_id=actor_id, **payload.model_dump())
