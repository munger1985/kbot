...
from platform_core.contracts.data_query import DataQueryPlanV1, normalize_catalog_dimension_filter
...
class SemanticDataQueryExecutor:
    ...
    async def _create_plan(self, *, context, question, models) -> DataQueryPlanV1:
        ...
        for attempt in range(2):
            ...
            try:
                ...
                self._validate_specialized_plan(
                    context=context,
                    consumer_app_id=str(agent.get("owner_app_id") or ""),
                    plan=plan,
                )
    ...
    def _validate_specialized_plan(
        *, context, consumer_app_id: str, plan: DataQueryPlanV1
        ...
        """通用问数计划不附加 App 业务约束。"""
        del context, consumer_app_id, plan
