...
class VisualSearchService:
    ...
    async def search_by_text(
        ...
        """文本 → embedding → Oracle VECTOR 搜索"""
        emb = await self._get_text_embedding(text)
        if not emb:
            return []
        return await self._search_by_embedding(emb, kb_ids, top_k, search_pages, search_images)
