...
class AssistantAgentEntity(BaseEntity):
    __tablename__ = "KBOT_ASST_AGENT"
    __table_args__ = (
        UniqueConstraint("domain_id", "display_name", name="UK_ASST_AGENT_NAME"),
        Index("IX_ASST_AGENT_SCOPE_STATUS", "domain_id", "status"),
    )
...
class AssistantAgentVersionEntity(BaseEntity):
    __tablename__ = "KBOT_ASST_AGENT_VERSION"
    __table_args__ = (
        UniqueConstraint("agent_id", "version_no", name="UK_ASST_AGENT_VERSION"),
    )

    agent_version_id: Mapped[UUID] = mapped_column(UUIDv7Type(), primary_key=True, default=uuid7)
    agent_id: Mapped[UUID] = mapped_column(UUIDv7Type(), nullable=False, index=True)
    version_no: Mapped[int] = mapped_column(Integer, nullable=False)
    knowledge_core_id: Mapped[UUID | None] = mapped_column(UUIDv7Type())
    data_model_ids_json: Mapped[list[str]] = mapped_column(OracleNativeJSON(), nullable=False)
