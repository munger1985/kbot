"""智能工作台 X Search / 文生图 Worker：认领租约后调用模型服务。"""
...
from assistant_app.application.runtime import (
    IMAGE_KIND, RESEARCH_KIND, TERMINAL_STATUSES, already_attempted, append_event,
    mark_upstream_attempted,
)
from assistant_app.entities import AssistantMediaAssetEntity, AssistantXSourceEntity
...
class AssistantRunWorker:
    ...
    async def run_forever(self) -> None:
        logger.info("智能工作台 Worker 开始运行：{}", self._worker_id)
        while True:
    ...
    async def process_once(self) -> bool:
        ...
        try:
            ...
            logger.exception("智能工作台 Run 处理失败：run_id={}", snapshot.run_id)
            await self._fail(
                snapshot,
                code="PROVIDER_UNAVAILABLE",
                message="处理 Run 时发生未预期错误",
            )
        return True
    ...
    async def _process(self, snapshot: RunSnapshot) -> None:
        async with self._uow_factory() as uow:
            ...
            if row.status in TERMINAL_STATUSES:
                return
            if already_attempted(row) and row.status not in TERMINAL_STATUSES:
                await append_event(
                    uow, row, stage="FAILED",
                    message="检测到未完成的上游尝试，已阻止二次调用",
                ...
                await uow.commit()
                return
            stage = "SEARCHING" if snapshot.kind == RESEARCH_KIND else "GENERATING"
            mark_upstream_attempted(row)
            await append_event(uow, row, stage=stage, message="正在调用模型服务")
            await uow.commit()

        try:
            if snapshot.kind == RESEARCH_KIND:
                result = await self._generative.research(self._research_request(snapshot))
                await self._complete_research(snapshot, result)
            elif snapshot.kind == IMAGE_KIND:
                result = await self._generative.generate_image(self._image_request(snapshot))
                await self._complete_image(snapshot, result)
            else:
                await self._fail(snapshot, code="PROVIDER_UNSUPPORTED_TOOL", message="不支持的 Run 类型")
        except GenerativeResponsesClientError as exc:
            await self._fail(snapshot, code=exc.code, message=exc.message)

    async def _complete_research(self, snapshot: RunSnapshot, result) -> None:
        async with self._uow_factory() as uow:
            row = await self._locked(uow, snapshot)
            if row is None:
                return
            ...
            if result.status != "COMPLETED":
                await append_event(
                    uow, row, stage="FAILED",
                    message=result.error_code or "X Search 未完成",
            ...
            for index, citation in enumerate(result.citations, start=1):
                await uow.x_sources.add(AssistantXSourceEntity(
                    source_id=uuid7(),
                    run_id=row.run_id,
                    domain_id=int(row.domain_id),
                    display_order=index,
                    citation_label=f"[X{index}]",
    ...
    async def _complete_image(self, snapshot: RunSnapshot, result) -> None:
        async with self._uow_factory() as uow:
            row = await self._locked(uow, snapshot)
            if row is None:
                return
            ...
            if result.status == "REJECTED":
                ...
                await append_event(uow, row, stage="REJECTED", message="内容安全策略拒绝生成")
                await uow.commit()
                return
            if result.status != "COMPLETED":
                row.error_code = result.error_code or "PROVIDER_UNAVAILABLE"
            ...
            for artifact in result.artifacts:
                ...
                await uow.media_assets.add(AssistantMediaAssetEntity(
                    asset_id=asset_id,
                    domain_id=int(row.domain_id),
                    run_id=row.run_id,
            ...
            await append_event(
                uow, row, stage="COMPLETED",
                message="图片已写入对象存储",
                payload={"artifact_count": len(asset_ids)},
            )
            await uow.commit()
    ...
    async def _fail(self, snapshot: RunSnapshot, *, code: str, message: str) -> None:
        async with self._uow_factory() as uow:
            ...
            await append_event(
                uow, row, stage="FAILED", message=message,
