...
from assistant_app.application.runtime import media_view, not_found, run_view
...
class MediaAssetService:
    ...
    def download_url(asset_id: UUID) -> dict[str, str]:
        return {
            "url": f"{PUBLIC_API_V1}/apps/assistant/media-assets/{asset_id}/content",
        }
    ...
    def _view(row) -> dict[str, Any]:
        return media_view(
            row,
            content_path=f"{PUBLIC_API_V1}/apps/assistant/media-assets/{row.asset_id}/content",
        )


class AssistantRunQueryService:
