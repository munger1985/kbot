...
from assistant_app.application.catalog import (
...
from assistant_app.entities import AssistantModelBindingEntity
from platform_core.identity import uuid7


BindingRole = Literal["KNOWLEDGE", "X_SEARCH", "IMAGE_GENERATION"]
...
def binding_view(row: AssistantModelBindingEntity) -> dict[str, Any]:
    ...
    return {
        ...
        "supports_x_search": bool(snapshot.get("supports_x_search")),
        "supports_image_generation": bool(snapshot.get("supports_image_generation")),
        "supports_responses_streaming": bool(snapshot.get("supports_responses_streaming")),
        "capability_verified_at": snapshot.get("capability_verified_at"),
...
class ModelBindingService:
    ...
    async def upsert(self, command: UpsertBindingCommand) -> dict[str, Any]:
        ...
        async with self._uow_factory() as uow:
            ...
            if row is None:
                row = AssistantModelBindingEntity(
                    binding_id=uuid7(),
