...
from core.dictionary import ToolType
...
class DifyAgent:
    ...
    async def _process_kb_tools(self, confs: list[Any], vector_search_question: str, full_text_question: str) -> list[KBSearchResult]:
        """
        处理知识库工具
        ...
        Returns:
            list[KBSearchResult]: 所有知识库搜索结果列表
        """
        ...
        if kb_tasks:
            ...
            # 处理搜索结果
            for conf, kb_result in zip(kb_configs, all_kb_results):
                if kb_result:
    ...
    async def search(self, question: str, override_question: bool = False) -> list[KBSearchResult]:
        ...
        Returns:
            list[KBSearchResult] | []: 知识库结果列表或空列表
        """
        ...
        logger.debug(f"找到 {len(confs)} 个知识库工具")
        
        # 4. 并行处理知识库工具
        kb_results = await self._process_kb_tools(confs, question, full_text_question) # type: ignore
