...
class MemoryService:
    ...
    async def prepare_context_and_rewrite(
        ...
        async with self.db_session as session:
            ...
            return {
                ...
                "new_state": new_state,
                "old_context": context,
                "current_plan": current_plan, # 传递给后续 Planner 判断是否是“追问”
                "active_topic": active_topic
            }
    ...
    async def persist_and_reflect_memory(
        ...
        async with self.db_session as session:
            ...
            try:
                # ========== 1. 从 context_memory 提取执行轨迹 (保持原逻辑但匹配新结构) ==========
                reasoning_path_structured = []
                # 兼容性处理：如果 execution_history 为空，尝试从 blocks 寻找轨迹
                ...
                if not turn_entities and context_memory.get("session_state"):
                    turn_entities = {
                        k: v for k, v in context_memory["session_state"].items()
                        if k not in ['history', 'messages', 'context']
                    }
                ...
                # 4.2 保存 Memory Entry (增加 blocks 的存储)
                ...
                # ========== 5. 反思逻辑 (使用新提取的 evidence_count) ==========
                user_profile = await self.get_user_profile(user_id)
                old_summary = user_profile.get("profile_summary", "新用户，暂无历史画像")
                ...
                new_summary, memory_snapshot = await self._do_llm_reflection(
