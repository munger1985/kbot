...
class OpsContextMemory(TypedDict):
    ...
    agent_id: int                  # 【⚠️ kbot3: int 类型】执行此任务的专职运维 Agent ID
    ...
    db_role: Literal["primary", "standby", "cluster_node"]  # 节点角色
    environment: Literal["prod", "stg", "dev"]  # 环境隔离标签
    ...
    # =========================================================================
    metric_results: list[dict[str, Any]]     # 合并后的唯一指标/数据沉淀区
    monitor_results: list[dict[str, Any]]    # Prometheus/Zabbix 监控数据沉淀区
