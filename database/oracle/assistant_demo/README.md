# Assistant App 问数 Demo CRM 数据

这里存放 Assistant App 问数 Demo 使用的外部 Oracle 数据库对象。它不是 KBot 内部
...
`documents/` 下的 PDF 与上述客户数据保持一致，可在后续 Assistant App/KC 流程中作为
用户资料上传。当前包含 7 份客户会议纪要和 1 份重点客户需求与风险摘要。可使用下面的
命令重新生成：

```bash
conda run -n cube python database/oracle/assistant_demo/generate_demo_pdfs.py
```
...
3. 在 Assistant App 的问数模型流程中创建外部 `ORACLE` 数据源：填写外部主机、端口、
