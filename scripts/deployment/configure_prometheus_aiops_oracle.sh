...
cat >"${override_candidate}" <<EOF
{
  "prometheus_queries": {
    ...
    "host.cpu.utilization": "100 - (avg by (target_key) (rate(node_cpu_seconds_total{job=\"node\",mode=\"idle\",target_key=\"\${external_target}\"}[5m])) * 100)",
    "host.memory.utilization": "100 * (1 - node_memory_MemAvailable_bytes{job=\"node\",target_key=\"\${external_target}\"} / node_memory_MemTotal_bytes{job=\"node\",target_key=\"\${external_target}\"})",
    "host.filesystem.utilization": "100 * (1 - node_filesystem_avail_bytes{job=\"node\",fstype!~\"tmpfs|overlay\",target_key=\"\${external_target}\"} / node_filesystem_size_bytes{job=\"node\",fstype!~\"tmpfs|overlay\",target_key=\"\${external_target}\"})",
    "host.disk.io.utilization": "rate(node_disk_io_time_seconds_total{job=\"node\",target_key=\"\${external_target}\"}[5m]) * 100",
    "host.network.throughput": "rate(node_network_receive_bytes_total{job=\"node\",device!=\"lo\",target_key=\"\${external_target}\"}[5m]) + rate(node_network_transmit_bytes_total{job=\"node\",device!=\"lo\",target_key=\"\${external_target}\"}[5m])"
  }
}
EOF
