...
class MemoryConsolidationWorker:
    ...
    async def _runtime_config(
        ...
        async with self._uow_factory() as uow:
            ...
            llm_model_name = str(agent.memory_llm_model_name or "").strip()
            if not llm_model_name:
                raise RuntimeError("Agent 未配置 memory_llm_model_name")
            raw = dict(agent.config_json or {}).get("memory") or {}
            if not isinstance(raw, dict):
                raise RuntimeError("Agent memory 配置必须是对象")
            embedding_model_name = str(
                agent.memory_embedding_model_name or ""
            ).strip() or None
            if not embedding_model_name:
                raise RuntimeError(
                    "Agent 未配置 memory_embedding_model_name"
                )
