# utils/clients/ops_db_client.py
...
from platform_core.config.settings import get_executor_config
...
class OpsDBExecutor:
    ...
    async def execute_readonly_ops_sql(
        ...
        # 指标查询直接向 Skill 返回纯净的原始行列 List[dict]
    ...
    async def _dispatch_to_ops_service(
        ...
        """
        # 1. 从物理运维资产中心（OPS DB）动态检索实例拓扑与认证元数据
        instance_meta = await self.ops_service.get_instance_by_id(instance_id)
        db_type = instance_meta["db_type"]                   # e.g., "oracle", "postgresql", "mysql"
        connection_config = instance_meta["connection_config"] # 包含 host, port, user, password
        environment = instance_meta.get("environment", "prod") # 环境：prod, stg, dev
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout, headers=headers) as session:
                logger.info(f"[OpsClient] 发起物理内核调用 | 实例: {instance_id} | 模式: {run_mode} | 环境: {environment}")
