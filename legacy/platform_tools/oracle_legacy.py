...
from platform_core.exceptions import DataNotFoundException, NotFoundError
...
try:
    ...
    logger.info("Async database engine initialized cleanly without global extensions.")
...
class DatabaseRuntime:
    """An App-owned engine and Session Factory.

    The module-level ``async_session`` remains only for the legacy runtime.
    New deployable Apps should create and inject one runtime during startup;
    changing the Settings database section is then sufficient to move that App
    to another database without changing repositories or UoW code.
    """
...
def create_database_runtime(settings: Settings | None = None) -> DatabaseRuntime:
    """Create an isolated database runtime for one deployable App."""
    config = settings or get_settings()
    oracle_config = config.oracle
    sqlalchemy_config = config.sqlalchemy
    runtime_url = (
        f"oracle+oracledb://{oracle_config.username}:{oracle_config.password}"
        f"@{oracle_config.host}:{oracle_config.port}/?service_name={oracle_config.service_name}"
    )
