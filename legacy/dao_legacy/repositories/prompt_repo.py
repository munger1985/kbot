...
from platform_core.config import get_app_config
from platform_core.dictionary import PromptCategory
    ...
    async def reset_sys_prompt(self) -> None:
        ...
        try:
            await self.session.execute(
                delete(PromptEntity)
                .where(PromptEntity.prompt_category == PromptCategory.SYSTEM_PROMPT.value)
            )
            ...
            await self.session.execute(
                delete(PromptEntity)
                .where(PromptEntity.prompt_unique_name == "TEMPLATE/Common_CN")
            )
            self.session.add(template_cn)

            template_en = PromptEntity(
                    app_id=get_app_config().app_id,
                    name="TEMPLATE/Common_EN",
                    prompt_unique_name="TEMPLATE/Common_EN",
                    prompt_category=PromptCategory.PROMPT_TEMPLATE.value,
                    template="Based on the above data and documents, directly answer the user's question. If there are contradictions between data and documents, please clearly point them out.",
                    status=1,
                    descs="Automatically generated by system.",
                    created_by="System",
                )
            await self.session.execute(
                delete(PromptEntity)
                .where(PromptEntity.prompt_unique_name == "TEMPLATE/Common_EN")
            )
